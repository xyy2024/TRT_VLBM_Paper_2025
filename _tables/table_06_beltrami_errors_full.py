...
FULL_ERROR_TABLE_FILE = TABLE_OUTPUT_DIR/'table_06_beltrami_errors_full.tex'
def write_full_tables(results):
    error_lines = [
        r'\begin{table}[htbp]', r'', r'\centering',
        r'\caption{Complete D3N7 ABC/Beltrami error data at $T=3/14$.  Brackets give the adjacent-grid order.}',
        r'\label{tab:numerical-beltrami-errors-full}', r'\begingroup',
    ...
    for choice, _ in RATE_CHOICES:
        ...
        for row in rows:
            entries = []
            for field_index, value in enumerate(row.component_error):
                if (
                    previous is None
